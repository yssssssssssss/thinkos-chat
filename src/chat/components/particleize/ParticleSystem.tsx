...
const ParticleSystem = forwardRef<ParticleSystemHandle, ParticleSystemProps>(
  ({ imageSrc, config, width, height }, ref) => {
    ...
    }, [imageSrc, config.particleDensity]);
      ...
      <div className="relative w-full h-full overflow-hidden flex items-center justify-center">
        {isProcessing && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-20">
            <div className="text-cyan-400 animate-pulse text-xl font-mono tracking-widest">正在分析像素...</div>
          </div>
        )}
        {!imageSrc && (
          <div className="absolute text-gray-600 font-mono text-sm pointer-events-none">无图像输入</div>
        )}
        <canvas
          ref={canvasRef}
